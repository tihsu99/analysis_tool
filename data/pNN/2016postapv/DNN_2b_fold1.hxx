//Code generated automatically by TMVA for Inference of Model file [DNN_2b_fold1.onnx] at [Wed Sep 25 03:12:19 2024] 

#ifndef TMVA_SOFIE_DNN_2B_FOLD1
#define TMVA_SOFIE_DNN_2B_FOLD1
...
namespace TMVA_SOFIE_DNN_2b_fold1{
namespace BLAS{
...
std::vector<float> fTensor_linearrelustack0weight = std::vector<float>(832);
...
Session(std::string filename ="") {
   if (filename.empty()) filename = "DNN_2b_fold1.dat";
   std::ifstream f;
   f.open(filename);
   if (!f.is_open()){
   ...
   if (length != 832) {
      std::string err_msg = "TMVA-SOFIE failed to read the correct tensor size; expected size is 832 , read " + std::to_string(length) ;
      throw std::runtime_error(err_msg);
    }
    for (int i =0; i < length; ++i) 
...
} //TMVA_SOFIE_DNN_2b_fold1

#endif  // TMVA_SOFIE_DNN_2B_FOLD1
