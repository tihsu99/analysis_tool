//Code generated automatically by TMVA for Inference of Model file [DNN_SR_3b3j_fold1.onnx] at [Tue Nov  5 15:21:41 2024] 

#ifndef TMVA_SOFIE_DNN_SR_3B3J_FOLD1
#define TMVA_SOFIE_DNN_SR_3B3J_FOLD1
...
namespace TMVA_SOFIE_DNN_SR_3b3j_fold1{
namespace BLAS{
...
std::vector<float> fTensor_linearrelustack6weight = std::vector<float>(512);
...
std::vector<float> fTensor_linearrelustack0weight = std::vector<float>(2688);
...
std::vector<float> fTensor_input11 = std::vector<float>(32);
...
Session(std::string filename ="") {
   if (filename.empty()) filename = "DNN_SR_3b3j_fold1.dat";
   std::ifstream f;
   f.open(filename);
   if (!f.is_open()){
   ...
   if (length != 2688) {
      std::string err_msg = "TMVA-SOFIE failed to read the correct tensor size; expected size is 2688 , read " + std::to_string(length) ;
      throw std::runtime_error(err_msg);
    }
    for (int i =0; i < length; ++i) 
...
//--------- Gemm
   ...
   int op_2_lda = 128;
   int op_2_ldb = 128;
   std::copy(tensor_linearrelustack3bias, tensor_linearrelustack3bias + 32, tensor_input7);
...
} //TMVA_SOFIE_DNN_SR_3b3j_fold1

#endif  // TMVA_SOFIE_DNN_SR_3B3J_FOLD1
