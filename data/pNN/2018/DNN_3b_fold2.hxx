//Code generated automatically by TMVA for Inference of Model file [DNN_3b_fold2.onnx] at [Wed Sep 25 03:12:14 2024] 

#ifndef TMVA_SOFIE_DNN_3B_FOLD2
#define TMVA_SOFIE_DNN_3B_FOLD2
...
namespace TMVA_SOFIE_DNN_3b_fold2{
namespace BLAS{
...
std::vector<float> fTensor_linearrelustack3weight = std::vector<float>(2048);
...
std::vector<float> fTensor_linearrelustack0weight = std::vector<float>(2560);
...
Session(std::string filename ="") {
   if (filename.empty()) filename = "DNN_3b_fold2.dat";
   std::ifstream f;
   f.open(filename);
   if (!f.is_open()){
   ...
   if (length != 2560) {
      std::string err_msg = "TMVA-SOFIE failed to read the correct tensor size; expected size is 2560 , read " + std::to_string(length) ;
      throw std::runtime_error(err_msg);
    }
    for (int i =0; i < length; ++i) 
...
} //TMVA_SOFIE_DNN_3b_fold2

#endif  // TMVA_SOFIE_DNN_3B_FOLD2
