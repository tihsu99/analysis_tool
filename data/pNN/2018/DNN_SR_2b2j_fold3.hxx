//Code generated automatically by TMVA for Inference of Model file [DNN_SR_2b2j_fold3.onnx] at [Tue Nov  5 15:21:38 2024] 

#ifndef TMVA_SOFIE_DNN_SR_2B2J_FOLD3
#define TMVA_SOFIE_DNN_SR_2B2J_FOLD3
...
namespace TMVA_SOFIE_DNN_SR_2b2j_fold3{
namespace BLAS{
...
struct Session {
std::vector<float> fTensor_linearrelustack9weight = std::vector<float>(16);
...
std::vector<float> fTensor_linearrelustack3bias = std::vector<float>(16);
float * tensor_linearrelustack3bias = fTensor_linearrelustack3bias.data();
std::vector<float> fTensor_linearrelustack3weight = std::vector<float>(1024);
...
std::vector<float> fTensor_linearrelustack6bias = std::vector<float>(16);
float * tensor_linearrelustack6bias = fTensor_linearrelustack6bias.data();
std::vector<float> fTensor_linearrelustack0weight = std::vector<float>(896);
...
std::vector<float> fTensor_input15 = std::vector<float>(16);
float * tensor_input15 = fTensor_input15.data();
std::vector<float> fTensor_input19 = std::vector<float>(16);
float * tensor_input19 = fTensor_input19.data();
std::vector<float> fTensor_input7 = std::vector<float>(16);
...
std::vector<float> fTensor_input11 = std::vector<float>(16);
...
Session(std::string filename ="") {
   if (filename.empty()) filename = "DNN_SR_2b2j_fold3.dat";
   std::ifstream f;
   f.open(filename);
   if (!f.is_open()){
   ...
   if (length != 896) {
      std::string err_msg = "TMVA-SOFIE failed to read the correct tensor size; expected size is 896 , read " + std::to_string(length) ;
      throw std::runtime_error(err_msg);
    }
    for (int i =0; i < length; ++i) 
...
//--------- Gemm
   ...
   int op_2_lda = 64;
   int op_2_ldb = 64;
   std::copy(tensor_linearrelustack3bias, tensor_linearrelustack3bias + 16, tensor_input7);
...
//--------- Gemm
   ...
   int op_4_lda = 16;
   int op_4_ldb = 16;
   std::copy(tensor_linearrelustack6bias, tensor_linearrelustack6bias + 16, tensor_input15);
...
} //TMVA_SOFIE_DNN_SR_2b2j_fold3

#endif  // TMVA_SOFIE_DNN_SR_2B2J_FOLD3
