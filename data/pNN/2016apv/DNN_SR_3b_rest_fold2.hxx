//Code generated automatically by TMVA for Inference of Model file [DNN_SR_3b_rest_fold2.onnx] at [Tue Nov  5 15:21:42 2024] 

#ifndef TMVA_SOFIE_DNN_SR_3B_REST_FOLD2
#define TMVA_SOFIE_DNN_SR_3B_REST_FOLD2
...
namespace TMVA_SOFIE_DNN_SR_3b_rest_fold2{
namespace BLAS{
...
std::vector<float> fTensor_linearrelustack0weight = std::vector<float>(1344);
...
Session(std::string filename ="") {
   if (filename.empty()) filename = "DNN_SR_3b_rest_fold2.dat";
   std::ifstream f;
   f.open(filename);
   if (!f.is_open()){
   ...
   if (length != 1344) {
      std::string err_msg = "TMVA-SOFIE failed to read the correct tensor size; expected size is 1344 , read " + std::to_string(length) ;
      throw std::runtime_error(err_msg);
    }
    for (int i =0; i < length; ++i) 
...
} //TMVA_SOFIE_DNN_SR_3b_rest_fold2

#endif  // TMVA_SOFIE_DNN_SR_3B_REST_FOLD2
