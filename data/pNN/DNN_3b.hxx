//Code generated automatically by TMVA for Inference of Model file [DNN_3b.onnx] at [Fri Mar 22 10:24:42 2024] 

#ifndef TMVA_SOFIE_DNN_3B
#define TMVA_SOFIE_DNN_3B
...
std::vector<float> fTensor_0linearrelustack6weight = std::vector<float>(32);
float * tensor_0linearrelustack6weight = fTensor_0linearrelustack6weight.data();
std::vector<float> fTensor_0linearrelustack3bias = std::vector<float>(32);
float * tensor_0linearrelustack3bias = fTensor_0linearrelustack3bias.data();
std::vector<float> fTensor_0linearrelustack3weight = std::vector<float>(2048);
...
std::vector<float> fTensor_0linearrelustack0weight = std::vector<float>(1216);
...
Session(std::string filename ="") {
   if (filename.empty()) filename = "DNN_3b.dat";
   std::ifstream f;
   f.open(filename);
   if (!f.is_open()){
   ...
   if (length != 1216) {
      std::string err_msg = "TMVA-SOFIE failed to read the correct tensor size; expected size is 1216 , read " + std::to_string(length) ;
      throw std::runtime_error(err_msg);
    }
    for (int i =0; i < length; ++i) 
...
//--------- Gemm
   ...
   int op_2_lda = 64;
   int op_2_ldb = 64;
   std::copy(tensor_0linearrelustack3bias, tensor_0linearrelustack3bias + 32, tensor_input7);
