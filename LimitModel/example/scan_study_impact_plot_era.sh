...
do
  ...
  do
    command_="sh example/plot_Impact_bHplus.sh '${command} --year ${era} --channel ${channel}' ${step_}" 
    echo -e "${BCyan}[tmux: ${era}_${channel}]${NC} ${BYellow} ${command_} ${NC}"
    tmux new-session -d -s $era\_$channel "${command_} ${MASS};"
  done
done
