import sys
import os, time, argparse, torch, shap, fnmatch, copy
...
def Evaluate(model, df, var, signal_mass, loss_function, device, pNN, test_id_ = None, outdir = './'):
  signal_mass.append(-1)
#  df,_ = train_test_split(df, test_size=0.0, shuffle=False)
  ...
  ax.scatter(bins[:-1]+ 0.5*(bins[1:] - bins[:-1]), bkg, marker='o', c='red', s=40, alpha=1, label = 'bkg (Validation)')
  ax.scatter(bins[:-1]+ 0.5*(bins[1:] - bins[:-1]), sig, marker='o', c='blue', s=40, alpha=1, label = 'sig (Validation)')
  ax.legend()
  plt.title("Distribution")
  plt.xlabel("DNN score")
  plt.ylabel("Density")
  plt.savefig(os.path.join(outdir, 'Distribution_validate_Mass{}.png'.format(signal_mass[0])))
  plt.savefig(os.path.join(outdir, 'Distribution_validate_Mass{}.pdf'.format(signal_mass[0])))
...
def Simple_Training(config, data):
  ...
  var = config["var"]
  model = DNN(len(var), config["l1"], config['l2'], config['l3'], config['dropout'])
...
def Training(indir, MVA_json, n_epoch, batch_size, signal_mass=[500], learning_rate=1e-3, k_folds=5, outdir='./', pNN=False, hyperparameter_tuning=False, ray_silence=False, MVA_Label='DNN_3b', bkg_cut = 999999, step = '1'):
  ...
  main_step = step if '_' not in step else step.split('_')[0]
  minor_step = '0' if '_' not in step else step.split('_')[1]
  print('main step: ', main_step)
  print('minor step: ', minor_step)
  ...
  df_majority = df_background if len(df_background) > len(df_signal) else df_signal
  df_minority = df_signal if len(df_background) > len(df_signal) else df_background
  # Undersample majority class
  df_majority_undersampled = resample(df_majority,
                                      replace=False,  # sample without replacement
                                      n_samples=min(bkg_cut * len(df_minority), len(df_majority)),  # to match minority class
                                      random_state=42)  # for reproducibility

  df = pd.concat([df_majority_undersampled, df_minority])


  train_df_raw, val_df = train_test_split(df, test_size=0.2, shuffle=True, random_state=42)
  train_df = train_df_raw[train_df_raw['Mass'].isin(signal_mass)]

  print('signal number: ', len(train_df[train_df['Label']==1]))
  print('background number: ', len(train_df[train_df['Label']==0]))
  ...
  ############################################
  ##  STEP 1 : Store hyperparameter Setting ##  
  ############################################

  best_config = {"l1":48, 'l2':24, 'weight_decay': 0.0, 'lr':learning_rate, 'batch_size':batch_size, 'dropout':0.0, 'l3':0}
  ...
  if main_step == "1":
    if hyperparameter_tuning:
      ...
      config = {
        ...
        'l3': tune.choice([0, 2, 4, 8, 16, 32, 64, 128]),
  ...
  #    os.system('ray status')
      ...
      tuner = tune.Tuner(
          tune.with_resources(
              tune.with_parameters(Simple_Training, data=train_df[:int(len(train_df)/4)]), # Only 25% of training data are used for hyperparameter tuning
  ...
  if main_step == '2':
    ...
    best_config = read_json(os.path.join(outdir, 'param.json'))
    model = DNN(len(var), best_config['l1'], best_config['l2'], best_config['l3'], best_config['dropout']).to(device)
    print(model)
    ...
    for fold, (train_id_, test_id_) in enumerate(kfold.split(train_df)):
      if not (str(fold + 1) == minor_step): continue
      model = DNN(len(var), best_config['l1'], best_config['l2'], best_config['l3'], best_config['dropout'])
      model.to(device)
      optimizer     = torch.optim.NAdam(model.parameters(), lr=learning_rate)
      ...
      train_result = {
        ...
        'ROC_fpr': list(fpr),
        'ROC_tpr': list(tpr),
        'ROC_thresholds': list(thresholds)
      }

      
      store_json(train_result, os.path.join(outdir, 'training_result_kfold{}.json'.format(fold)))
      torch.save(model, os.path.join(outdir, 'model_fold{}.pt'.format(fold)))
      ...
      plt.savefig(os.path.join(outdir,'shap_vals_{fold}.png'.format(fold=fold)))
      plt.savefig(os.path.join(outdir,'shap_vals_{fold}.pdf'.format(fold=fold)))
      ...
      plt.savefig(os.path.join(outdir, 'Distribution_fold{}.png'.format(fold)))
      plt.savefig(os.path.join(outdir, 'Distribution_fold{}.pdf'.format(fold)))

      return 0
#      models.append(model)
#      test_loss_list.append(test_losses[fold][-1])
  ...
  if main_step == '3':
      ...
      for fold in range(k_folds):
        train_result = read_json(os.path.join(outdir, 'training_result_kfold{}.json'.format(fold)))
        ax.plot(train_result['train_losses'], c='blue', alpha=0.5, label='train(fold {})'.format(fold), marker = marker_list[fold])
        ax.plot(train_result['test_losses'],  c='orange', alpha=0.5, label='test(fold {})'.format(fold), marker = marker_list[fold])
        ax.legend(loc="upper right")
        performance[fold] = train_result['test_losses'][-1]
        test_loss_list.append(train_result['test_losses'][-1])
        model_name = os.path.join(outdir, 'model_fold{}.pt'.format(fold))
        model = torch.load(model_name, map_location=torch.device('cpu'))
        models.append(copy.deepcopy(model))
        train_result_kfold[fold] = train_result
      fig.savefig(os.path.join(outdir, 'loss_vs_epoch.png'))
      fig.savefig(os.path.join(outdir, 'loss_vs_epoch.pdf'))
      model = Ensemble_DNN(models, test_loss_list)
      torch.save(model, os.path.join(outdir, 'model.pt'))
      model.to(device)
      mean_auc = ROC_kfold_from_train_result(train_result_kfold, os.path.join(outdir, 'ROC.png'))
      mean_auc = ROC_kfold_from_train_result(train_result_kfold, os.path.join(outdir, 'ROC.pdf'))
      #mean_auc = ROC_kfold(Y_true, Y_pred, W, os.path.join(outdir, 'ROC.png'))
      #mean_auc = ROC_kfold(Y_true, Y_pred, W, os.path.join(outdir, 'ROC.pdf'))
      ...
      for mass_ in all_mass:
        if mass_ in signal_mass and len(signal_mass)==2: 
          ...
          AUC_across_signal[mass_] = Evaluate(model, val_df, var, [mass_], loss_function, device, pNN, outdir=outdir)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--MVA_Label', type=str)
  parser.add_argument('--bkg_cut', type=int, default = 99999) # Default to be equalivalent to no cut.
  parser.add_argument('--step', type=str, default = "1")
  args = parser.parse_args()
  if not os.path.exists(args.outdir):
    os.system('mkdir -p {}'.format(args.outdir))
  Training(args.indir, args.MVA_json, args.n_epoch, args.batch_size, signal_mass=args.Masses, k_folds=args.k_folds, outdir=args.outdir, pNN=args.pNN, hyperparameter_tuning=args.hyperparameter_tuning, ray_silence=args.ray_silence, MVA_Label = args.MVA_Label, bkg_cut = args.bkg_cut, step = args.step)
